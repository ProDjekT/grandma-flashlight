...
On each push to `master`, GitHub Actions runs tests and builds the debug APK (using `gradle/actions/setup-gradle` and the Android SDK setup action). The APK is available as an artifact of the workflow run (Actions → select run → Artifacts → app-debug).
